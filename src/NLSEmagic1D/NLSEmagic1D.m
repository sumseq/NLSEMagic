...
%Originally developed with support from the
...
endtw        = 10;  %End time of simulation.
numframes    = 20;  %Number of desired frames to plot.
h            = 1/70;%Spatial grid spacing.
k            = 0;   %Time step [Set to 0 to auto-compute smallest stable timestep]
method       = 2;   %Method:  1: CD O(4,2), 2: 2SHOC O(4,4)
cuda         = 1;   %Use NVIDIA GPU-acceleration CUDA codes (1) or CPU (0)
precision    = 1;   %Single (1) or double (2) precision.
...
BC   = 1;   %Boundary conditions Dirichlet: (1)  MSD (2)
...
%Set up CUDA info and check for MSD BC
if(cuda==1)
   cudablocksize     = 512;
...
%Report wall time:
twall = toc(twall);
disp(['NLSEmagic1D Total Wall Time: ',num2str(twall),' seconds.']);
%%%%%%%%%                                                 %%%%%%%%%
%%%%%%%%%         SIMULATION IS OVER                      %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
