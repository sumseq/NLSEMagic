...
#----------------------------------------------------------------------------
            ...
            CC='nvcc'
            CFLAGS='-m32 -gencode=arch=compute_30,code=sm_30 -gencode=arch=compute_30,code=compute_30 -gencode=arch=compute_20,code=compute_20 -gencode=arch=compute_20,code=compute_20 -O3 -Xcompiler "-fPIC -D_GNU_SOURCE -pthread -fexceptions"'
...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='nvcc'
            CFLAGS='-gencode=arch=compute_30,code=sm_30 -gencode=arch=compute_30,code=compute_30 -gencode=arch=compute_20,code=compute_20 -gencode=arch=compute_20,code=compute_20 -O3 -Xcompiler "-fPIC -D_GNU_SOURCE -pthread -fexceptions -m64"'
            CLIBS="$RPATH $MLIBS -lm -lstdc++"
            COPTIMFLAGS='-Xcompiler "-O3 -funroll-loops -msse2 -DNDEBUG"'
            CDEBUGFLAGS='-g'
#
...
#
            LD="$COMPILER"
            LDEXTENSION='.mexsol'
            LDFLAGS="-shared -Wl,-R,$GCC_LIBDIR"
            LDOPTIMFLAGS='-O'
            LDDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':'
