...
hv             = [1/2];%Spatial step-size.
%CD: 
%klin  = 0.009428090415821
%kfull = 0.008649624234698
%2SHOC:
%klin  = 0.007071067811865
%kfull = 0.006623951111818
...
if(IC==1)   %Linear EXP
    ...
    a  = 1; 
    BC = 1;
    ymax = 0.5*sqrt(-2*a*log(sqrt(eps)));
...
if(save_images==1)
    ...
    if(plot_3D_vol==1)
        if(plot_modulus2==1)
           ...
           print('-djpeg','-r150','-opengl',[fn, '_T0', '_MOD2_3DVOL'],['-f',num2str(fig_cube_mod2)]);            
        end
        if(plot_vorticity==1)
           set(fig_cube_vort, 'PaperPositionMode', 'auto'); 
           set(fig_cube_vort,'InvertHardcopy','off');
           fignum = ['-f',num2str(fig_cube_vort)];
           print('-djpeg','-r150',[fn, '_T0','_VORT_3DVOL'],fignum);     
        end
    end
    if(plot_2D_cuts==1)
        if(plot_modulus2==1)
           ...
           print('-depsc','-r150',[fn, '_T0','_MOD2_2DCUTS'],fignum); 
           print('-djpeg','-r150',[fn, '_T0','_MOD2_2DCUTS'],fignum); 
        end
        if(plot_vorticity==1)
           set(fig_cuts_vort, 'PaperPositionMode', 'auto'); 
           set(fig_cuts_vort,'InvertHardcopy','off');
           fignum = ['-f',num2str(fig_cuts_vort)];
           print('-depsc','-r150',[fn, '_T0','_VORT_2DCUTS'],fignum);   
           print('-djpeg','-r150',[fn, '_T0','_VORT_2DCUTS'],fignum); 
        end
        if(plot_phase==1)
           set(fig_cuts_phase, 'PaperPositionMode', 'auto'); 
           set(fig_cuts_phase,'InvertHardcopy','off');
           fignum = ['-f',num2str(fig_cuts_phase)];
           print('-depsc','-r150',[fn, '_T0','_PHASE_2DCUTS'],fignum); 
           print('-djpeg','-r150',[fn, '_T0','_PHASE_2DCUTS'],fignum); 
        end
     end    
end
...
if(plot_3D_vol==1)
    if(plot_modulus2==1)
        ...
        if(save_images==1)            
            set(fig_cube_mod2,'InvertHardcopy','off')
            set(fig_cube_mod2, 'PaperPositionMode', 'auto'); 
            print('-djpeg','-r150','-opengl',[fn,'_n',num2str(n), '_MOD2_3DVOL_t=' strrep(num2str(n*k),'.','')],['-f',num2str(fig_cube_mod2)]);   
        end
        if(save_movies==1 || save_movies==3)
            gifname = [fn,'_MOD2_3DVOL.gif'];
            I = getframe(fig_cube_mod2);
    ...
    if(plot_vorticity==1)         
       ...
       if(save_images==1)
            ...
            set(fig_cube_vort, 'PaperPositionMode', 'auto');       
            fignum = ['-f',num2str(fig_cube_vort)];
            print('-djpeg','-r150',[fn,'_n',num2str(n), '_T', strrep(num2str(n*k),'.',''), '_VORT_3DVOL'],fignum);  
       end
       if(chunk_count==numframes)
...
%Plot ring center-position:
   ...
   if(save_plots==1) 
      ...
      print('-djpeg','-r150',fndvt,['-f',num2str(figrad)]); 
      print('-depsc','-r150',fndvt,['-f',num2str(figrad)]); 
...
%    end 
   ...
   if(save_plots==1) 
      ...
      fnpos = ['ZvsY',fnbase];
      set(gcf, 'PaperPositionMode', 'auto');       
      print('-djpeg','-r150',fnpos,['-f',num2str(figpos)]); 
      print('-depsc','-r150',fnpos,['-f',num2str(figpos)]); 
   ...
   if(save_plots==1) 
      ...
      fnzct = ['ZvsT',fnbase];
      set(gcf, 'PaperPositionMode', 'auto');       
      print('-djpeg','-r150',fnzct,['-f',num2str(figzct)]); 
      print('-depsc','-r150',fnzct,['-f',num2str(figzct)]); 
   ...
   if(save_plots==1) 
      ...
      fnzct = ['YvsT',fnbase];
      set(gcf, 'PaperPositionMode', 'auto');       
      print('-djpeg','-r150',fnzct,['-f',num2str(figyvt)]); 
      print('-depsc','-r150',fnzct,['-f',num2str(figyvt)]); 
...
if(track_ring==1 && length(rmaxv)>1)   
  ...
  figure(fig_zvel)
  set(gcf, 'PaperPositionMode', 'auto');       
  print('-djpeg','-r150',['VRzvel_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(d),'.jpg'],['-f',num2str(fig_zvel)]); 
  print('-depsc','-r150',['VRzvel_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(d),'.eps'],['-f',num2str(fig_zvel)]); 
 
  figure(fig_zvelerr)
  set(gcf, 'PaperPositionMode', 'auto');       
  print('-djpeg','-r150',['VRzvelerr_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(d),'.jpg'],['-f',num2str(fig_zvelerr)]); 
  print('-depsc','-r150',['VRzvelerr_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(d),'.eps'],['-f',num2str(fig_zvelerr)]); 
 end  
end %track and rmaxv
...
if(length(rmaxv)>1)    
  if(length(BCv)==2)  
    ...
    % hold on;
     ...
     set(gcf, 'PaperPositionMode', 'auto');       
     print('-djpeg','-r150',['VRzvelBC_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(d),'.jpg'],['-f',num2str(fig_zvelbc)]); 
     print('-depsc','-r150',['VRzvelBC_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(d),'.eps'],['-f',num2str(fig_zvelbc)]); 
  end      
end %length(rmaxv)>!
...
if(track_ring==1 && ((length(dv)>1 && length(rmaxv)==1) && IC==3))    
     ...
     if(save_plots==1) 
     set(gcf, 'PaperPositionMode', 'auto');       
     print('-djpeg','-r150',['VRzvel_d_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(dv(1)),'-',num2str(dv(end)),'_q',num2str(q),'.jpg'],['-f',num2str(fig_zvel_d)]); 
     print('-depsc','-r150',['VRzvel_d_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(dv(1)),'-',num2str(dv(end)),'_q',num2str(q),'.eps'],['-f',num2str(fig_zvel_d)]); 
     end
    ...
    % hold off;
     ...
     if(save_plots==1) 
     set(gcf, 'PaperPositionMode', 'auto');       
     print('-djpeg','-r150',['VRzvelerr_d_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(dv(1)),'-',num2str(dv(end)),'_q',num2str(q),'.jpg'],['-f',num2str(fig_zvel_derr)]); 
     print('-depsc','-r150',['VRzvelerr_d_T',num2str(endt),'_m',num2str(method),'_h',num2str(h),'_d',num2str(dv(1)),'-',num2str(dv(end)),'_q',num2str(q),'.eps'],['-f',num2str(fig_zvel_derr)]);   
     end
end
...
if(length(numframesv)>1)
   ...
   if(save_plots==1) 
   set(gcf, 'PaperPositionMode', 'auto');       
   print('-djpeg','-r150',['3D_T',num2str(endt),'_N',num2str(min(xreswantedv)),'_',num2str(max(xreswantedv)),'_M',num2str(method),'.jpg'],['-f',num2str(fig_frames)]); 
   print('-depsc','-r150',['3D_T',num2str(endt),'_N',num2str(min(xreswantedv)),'_',num2str(max(xreswantedv)),'_M',num2str(method),'.eps'],['-f',num2str(fig_frames)]); 
   end
end
end %cuda
