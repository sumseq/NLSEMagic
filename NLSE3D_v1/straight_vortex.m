...
ICv            = [4];    %Select intial condition (1: linear guassian, 2: bright vortices, 3: dark vortices).
...
BCv            = [2];    %Overides boundary condition choice of IC (0:use IC BC 1:Dirichlet 2:MSD 3:Lap0.
...
high_qual      = 1;      %1: Use high-quality volumetric rending (slower). 0: don't.
...
if(IC>1 && IC<3)
    ...
    for vi=1:num_vort
        if(IC==2)
            ...
            if(rmaxv(1)==0)
               rmax  = R0 + 5;%(1/C)*asech(sqrt(eps)/B) + R0;
            else
               rmax = R0 + rmaxv(rmaxi);
            end
        elseif(IC==3)
...
% MARCO
ap(1) = 11/32;
bp(1) = (5-32*ap(1))/(48-192*ap(1));
ap(2) = ap(1)*(bp(1) - 1/4);
r2 = X .^ 2 + Y .^ 2;
rho = r2 .* (ap(1)+ap(2)*r2) ./ ((ap(2)*r2+bp(1)) .* r2 + 1);
theta = atan2(Y,X);
U     = sqrt(rho) .* exp(1i*theta) + 1;
% MARCO END
end
