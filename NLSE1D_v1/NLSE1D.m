%NLSE1D  Full Research Driver Script
...
endtw        = 100;     %Desireed end time fo the simulation - may be slightly altered.
...
%CD:
%kfull = 0.028284271263023
%klin  = 0.028284271247462
%2SHOC:
%klin  = 0.021213203435596
%kfull = 0.021213203444349
k            = 0;      %Time step-size.  Set to 0 to auto-compute largest stable size.
ICv          = [2];    %Select intial condition (1: bright soliton, 2: dark, 3: linear gaussian, 4: gray soliton)
...
xreswantedv  = [0];    %Desired grid size (N) (Overwrites IC auto grid-size calculations, and may be slightly altered)
cudav        = [0];    %Use CUDA integrators (0: no, 1: yes) 
precisionv   = [2];    %Use single (1) or double (2) precision integrators.
BCv          = [2];    %Overides boundary condition choice of IC (0:use IC BC 1:Dirichlet 2:MSD 3:Lap0 4:1-side 5:exact (IC3 only)
...
if(IC==1)%Bright sech soliton
   ...
   s    = 1;   
   OM   = 0.5;   %Frequency
   c    = 0.0;  %Velocity of soliton
   x0   = 0;
   %Compute numerical domain to be big enough to avoid BC effects:
   xmin = -0.25*sqrt(a/OM)*asech(eps*sqrt(s/(2*OM)));
   xmax =  0.25*sqrt(a/OM)*asech(eps*sqrt(s/(2*OM))) + c*endtw;
   BC   = 1;   %Dirchilet BC
elseif(IC==2)%Tanh dark soliton
   a    =  1;
   s    = -1;  
   OM   = -1;   %Frequency
   c    = 0.25; %Velocity of soliton
   x0   = 0;
   %Compute numerical domain to be big enough to avoid BC effects:
   xmin = -sqrt(abs((2*a)/OM))*atanh(1-(eps)*sqrt(s/OM));
   xmax =  20;%sqrt(abs((2*a)/OM))*atanh(1-(eps)*sqrt(s/OM)) + c*endtw;
...
elseif(IC==4)%Grey soliton
   ...
   xmax =  20;%sqrt(abs((2*a)/OM))*atanh(1-sqrt(eps)*sqrt(s/OM)) + c*endtw;
   BC   = 2;   %Modulus-Squared Dirchilet BC
end
...
if(k==0)  
   ...
   elseif(BC==2)           
       UB_1  = [U(2);U(end-1)];
       Ut    = NLSE1D_F(U,V,hmin,s,a,method,BC,(1/hmin^2),(a/12),(a/(6*hmin^2)),(1/a));
       UtB_1 = [Ut(2);Ut(end-1)];                     
...
maxRealU  = max(real(U));
plotmax   = 2;%max([maxMod2U maxRealU]);

%Set up plot of initial condition:
fig_plot  = figure(fig_count);
fig_count = fig_count+1;
set(fig_plot, 'Name','1D NLSE  t = 0','NumberTitle','off');
plot_real = plot(xvec,real(U),   '-b' ,'LineWidth',3); 
hold on;
plot_v = plot(xvec,V,   '-g','LineWidth',3);
plot_imag = plot(xvec,imag(U),   '-r' ,'LineWidth',3);
...
elseif(method==2)   
    ...
    %Start Runga-Kutta:     
    k_tot  = NLSE1D_F(U,V,h,s,a,method,BC,l_a,l_h2,l76,l_12,par); %K1   
    %----------------------------     
    Uk_tmp = U + k2*k_tot;    
    par(1) = k*(n+nc-1) + k2;
    Uk_tmp  = NLSE1D_F(Uk_tmp,V,h,s,a,method,BC,l_a,l_h2,l76,l_12,par); %K2
    k_tot = k_tot + 2*Uk_tmp; %K1 + 2K2
    %----------------------------        
    Uk_tmp  = U + k2*Uk_tmp;     
    Uk_tmp  = NLSE1D_F(Uk_tmp,V,h,s,a,method,BC,l_a,l_h2,l76,l_12,par); %K3
    k_tot = k_tot + 2*Uk_tmp; %K1 + 2K2 + 2K3
    %----------------------------
    Uk_tmp  = U + k*Uk_tmp;  
    par(1)  = k*(n+nc) + k;
    k_tot   = k_tot + NLSE1D_F(Uk_tmp,V,h,s,a,method,BC,l_a,l_h2,l76,l_12,par); %K1 + 2K2 + 2K3 + K4
    %-------------------------------
